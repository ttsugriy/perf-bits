...
> **For the best reading experience with properly rendered equations, [view this article on GitHub Pages](https://ttsugriy.github.io/perf-bits/quantization.html).**
...
**Over-parameterization ⟹ Redundancy ⟹ Quantization tolerance**
...
**Gradient noise during training**: variance σ² ∝ η/B

**Quantization noise**: variance σ² ∝ Δ²/12

If Δ is small enough that quantization noise is comparable to training noise, accuracy is preserved.

### 2. Flat Minima Are Robust

Modern neural networks converge to flat minima—regions where the loss changes slowly with parameters.
...
- Low-rank approximations capture most of the information
...
|--------|------|-------|-----------|----------|
| FP32 | 32 | ±3.4 × 10³⁸ | ~7 decimal digits | Training (legacy) |
| BF16 | 16 | ±3.4 × 10³⁸ | ~3 decimal digits | Training (modern) |
| FP16 | 16 | ±65504 | ~4 decimal digits | Mixed precision |
...
**Symmetric quantization**: zero point = 0, range is [-α, α]
**Asymmetric quantization**: zero point ≠ 0, range is [β_min, β_max]
...
**KV cache size = 2 × L × n_heads × d_head × seq_len × bytes**
...
n-bit multiplication requires O(n²) circuit area. INT8 uses 25% of the area of FP32 multiply. The savings compound.
...
**If Δw < quantization step ⟹ w_new = w_old**
...
*Previous article: [Compositionality: The Power of Depth](https://ttsugriy.github.io/perf-bits/compositionality.html)*
